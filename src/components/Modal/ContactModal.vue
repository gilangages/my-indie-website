...
const handleClose = async () => {
  ...
  if (!clickAudio) clickAudio = new howlerModule.Howl({ src: [clickSfxFile], volume: 0.1 });
  clickAudio.stop();
  clickAudio.play();
  emit("close");
};
      ...
      <div v-show="open" class="fixed inset-0 z-50 flex justify-center pointer-events-none"
        ...
        <transition :name="isMobile ? 'sheet' : 'scale'">
          <div v-show="open" :class="isMobile
            ...
            ">
            <div
              class="flex justify-between text-accent items-center p-3 sm:p-4 z-20 relative transition-colors duration-300">
              <p>contact</p>
              <button
                class="text-2xl transition-transform duration-200 hover:scale-115 cursor-pointer bg-transparent border-none pr-3"
                @click="handleClose">
                {{ isMobile ? "∨" : "[x]" }}
              </button>
            </div>
            ...
            <div class="px-4 custom-scroll flex flex-col items-center text-center w-full"
              ...
              <div class="flex justify-center w-full relative z-0">
                <img :src="bannerImg" alt="Banner" class="relative object-contain transition-all duration-300" :class="isMobile ? 'w-[100px] -mt-8 translate-y-4' : 'w-full max-w-[600px] max-h-[50vh] -translate-y-4'
                  " />
              </div>
